...
class QLDTApi:
    def __init__(self):
        self.base_url = "https://qldt.ptit.edu.vn"
    ...
    def get_registered_courses(self, token):
        ...
        data = self._post(
            '/public/api/dkmh/w-locdskqdkmhsinhvien',
    ...
    def register_course(self, token, id_to_hoc):
        """Đăng ký một môn học"""
        return self._post(
            '/dkmh/api/dkmh/w-xulydkmhsinhvien',
    ...
    def logout(self, token):
        ...
        try:
            return self._post('/public/api/auth/logout', json={}, token=token)
        except:
...
with st.sidebar:
    ...
    st.write("**Server:** qldt.ptit.edu.vn")
